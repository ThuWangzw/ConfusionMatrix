...
import math
import copy
# import backend.data.reordering as reordering
...
def test():
    children_number = 6
    A = getCombinations(list(range(children_number)), math.ceil(children_number / 2))
    print(A)
    ...
    test_m = [[100 for _ in range(15)] for _ in range(15)]
    for i in range(14):
        test_m[i][i+1] = 10
        test_m[i+1][i] = 10
    ...
    n12 = TreeNode(12)
    n13 = TreeNode(13)
    tree.children = [n0, n1, n2, n3, n4, n5]
    # n0.children = [n10, n11, n12, n13]
    # tree.children = [n0, n3, n2, n1]
    ...
    for i in range(14):
        for j in range(14):
            if tuple([i, j]) in kolo.M:
                print(i, j, kolo.M[tuple([i, j])])
...
def test_on_dataset():
    import os
    import json
    evaluationPath = os.path.join('/data/zhaowei/jittor-data/', "evaluation.json")
    with open(evaluationPath, 'r') as f:
        statisticData = json.load(f)
    confusion = statisticData["confusion"]
    CM = confusion['matrix']
    names = confusion['names']
    hierarchy = confusion['hierarchy']

    from reorder import getSimilarityMatrix, hierarchyToTree, setIds, setLeaves, treeToHierarchy
    ...
    kolo = KOLO(tree, SM)
    opt_tree, ord_ids = kolo.getOrdered()
