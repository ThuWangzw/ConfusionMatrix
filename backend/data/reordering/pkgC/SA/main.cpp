...
constexpr int NLEVEL = 6;
long long level_dis_mat[NLEVEL][N][N];
int A[NLEVEL][N][N];
...
double calc_agg_cost(long long level_dis_mat[][1005][1005], double gamma, Node* tree) {
    vector<vector<Node*>> levels;
    get_levels(levels, tree);
    double cost = 0;
    double times = 1;
    ...
    for (int l = num_levels - 1; l > 0; l--) {
        int n = levels[l].size();
        //for (int i = 0; i < levels[l].size(); i++) ids.push_back(levels[l][i]->id);
        // for (auto& ptr : levels[l]) ids[i] = (ptr->id);
        for (int i = 0; i < levels[l].size(); i++) ids[i] = levels[l][i]->id;
        long long level_cost = 0;
#pragma omp parallel for reduction (+:level_cost)
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                //level_cost += a[i][j] * level_dis_mat[l][ids[i]][ids[j]];
                level_cost += A[l][i][j] * level_dis_mat[l][ids[i]][ids[j]];
            }
        }
 //       cout << level_cost << "\n";
        // cost += times * level_cost;
...
vector<double> calc_level_bar(long long level_dis_mat[][1005][1005], double gamma, Node* tree) {
    vector<vector<Node*>> levels;
    get_levels(levels, tree);
    double cost = 0;
    double times = 1;
    ...
    for (int l = num_levels - 1; l > 0; l--) {
        ...
        //for (int i = 0; i < levels[l].size(); i++) ids.push_back(levels[l][i]->id);
        long long level_cost = 0;
        for (int i = 0; i < n; i++) ids[i] = levels[l][i]->id;
...
void annealing_hierarchy(int n, mat& a, mat& b, vector<int>& best_perm, double& best_cost, int num_iters, int num_init_iters, Node* tree, double gamma, bool preordered)
{
    ...
    double dmin = std::numeric_limits<double>::max(), dmax = 0;
    ...
    if (dmin == std::numeric_limits<double>::max()) cout << "DBL_MAX" << endl;
